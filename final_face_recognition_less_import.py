...
from tkinter import LEFT, Button, Label, Tk, Frame, Canvas, BOTTOM
from cv2 import rectangle, cvtColor, COLOR_BGR2RGB, VideoCapture
...
def open_window1_face_encoder(person_id_, person_name_, user_):
    ...
    def update_face_canvas():
        ...
        for i in range(len(latest_faces)):
            face_image = cvtColor(latest_faces[i], COLOR_BGR2RGB)
            face_image = Image.fromarray(face_image)
            face_image = ImageTk.PhotoImage(face_image)

            label = Label(face_canvas, image=face_image)
            ...
            compare_button = Button(face_canvas, text="บันทึกภาพใบหน้า", font=("Arial", 10), fg="#fff", bg="#555",
                                       command='', name=str(i))
            compare_button.pack(side=LEFT, padx=3, pady=3, ipadx=3)
    ...
    title_label = Label(window1, text="บันทึกภาพใบหน้า", font=("Arial", 16, "bold"), fg="#555", pady=10)
    title_label.pack()
    ...
    video_canvas = Canvas(frame, width=640, height=480)
    video_canvas.pack(side=LEFT, padx=10)

    # Create the save button
    save_button = Button(frame, text="บันทึกภาพใบหน้า", font=("Arial", 14), fg="#fff", bg="#555", command=save_face)
    # save_button.pack(side=tk.LEFT)

    # Create a frame for the face image
    face_frame = Frame(window1)
    face_frame.pack(pady=10)

    # Create a label for the face image
    face_label = Label(face_frame, text="ภาพใบหน้าที่บันทึกได้", font=("Arial", 14, "bold"), fg="#555", pady=10)
    face_label.pack()

    # Create the face canvas
    face_canvas = Canvas(face_frame, width=200, height=200)
    face_canvas.pack()
...
def open_window2_face_comparison():
        ...
        if mydb.is_connected():
            try:
                ...
                if len(show_label['id']) > 0:
                    ...
                    label_person_info = Label(face_frame, text='ไม่พบบุคคลในฐานข้อมูล', name='result0', font=("Arial", 14))
                    label_person_info.pack(side=BOTTOM, pady=10)
    ...
    def update_face_canvas():
        ...
        face_image = cvtColor(face_image, COLOR_BGR2RGB)
        face_image = Image.fromarray(face_image)
        face_image = ImageTk.PhotoImage(face_image)

        label = Label(face_canvas, image=face_image, name='')
        label.image = face_image
        label.pack(side=LEFT, padx=10)
        # label.bind("<Button-1>", label_click)
        ...
        if not compare_button_exists:
            compare_button = Button(face_canvas, text="ค้นหาจากใบหน้า", font=("Arial", 14), fg="#fff", bg="#555",
                                       command=compare_face, name="compare_button")
            compare_button.pack(side=LEFT, padx=10, pady=10, ipadx=10)
    ...
    # Create a title label
    title_label = Label(window2, text="ค้นหาบุคคลจากใบหน้า", font=("Arial", 16, "bold"), fg="#555", pady=10)
    title_label.pack()
